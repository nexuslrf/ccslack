...
with handlers and tests that import from ``ccslack.providers.shell``.
...
- ``replace``: replaces the entire prompt with ``{prefix}:N❯``
  (the legacy behaviour, opt-in via ``CCSLACK_PROMPT_MODE=replace``).
"""

from typing import Any, ClassVar

from ccslack.providers._jsonl import JsonlProvider
from ccslack.providers.base import ProviderCapabilities

# Re-exports for backward compat — new code should import directly from
# ccslack.providers.shell_infra. Listed in __all__ so star-imports continue to
# work and tools see the public surface.
from ccslack.providers.shell_infra import (  # noqa: F401
    KNOWN_SHELLS,
    PromptMatch,
    detect_pane_shell,
    get_shell_name,
