...
that handlers and the rest of ccslack need to drive a shell session
    ...
    if mode not in _VALID_PROMPT_MODES:
        if not _WARNED_INVALID_MODE:
            ...
            structlog.get_logger().warning(
                "Invalid CCSLACK_PROMPT_MODE=%r, defaulting to 'wrap'", mode
            )
        return "wrap"
    return mode
...
def _wrap_setup_commands(shell: str) -> str:
    ...
    fish = (
        "builtin functions --query __ccslack_orig_prompt; or begin; "
        "builtin functions --copy fish_prompt __ccslack_orig_prompt 2>/dev/null; "
        "or function __ccslack_orig_prompt; end; "
        "function fish_prompt; "
        "set -l __s $status; "
        "__ccslack_orig_prompt; "
        "set_color brblack; printf '⌘%d⌘ ' $__s; set_color normal; "
        "end; clear; end"
    )
    bash = (
        "type __ccslack_sc >/dev/null 2>&1 || { "
        "__ccslack_sc(){ __ccslack_x=$?; return $__ccslack_x; }; "
        'PROMPT_COMMAND="__ccslack_sc${PROMPT_COMMAND:+;$PROMPT_COMMAND}"; '
        'PS1="${PS1}\\[\\033[2m\\]⌘\\${__ccslack_x}⌘\\[\\033[0m\\] "; '
        "clear; }"
    )
    zsh = (
        '[[ "$PROMPT" == *⌘%\\?⌘* ]] || { '
...
async def setup_shell_prompt(
    ...
    *,
    ...
    # Never send prompt setup to ccslack's own window — the C-c would kill the bot
