...
so consumers can do ``from ccslack.providers import registry, ...``.
...
from ccslack.expandable_quote import EXPANDABLE_QUOTE_END, EXPANDABLE_QUOTE_START
...
from ccslack.providers.process_detection import JS_RUNTIMES
from ccslack.providers.registry import ProviderRegistry, UnknownProviderError, registry
...
def _ensure_registered() -> None:
    ...
    # Lazy: provider classes register against the registry at import; defer until the registry factory runs
    from ccslack.providers.gemini import GeminiProvider

    # Lazy: provider classes register against the registry at import; defer until the registry factory runs
    from ccslack.providers.pi import PiProvider
...
def detect_provider_from_runtime(
    ...
    *,
    ...
    # Check for ccslack title stamp (set on launch via stamp_pane_title)
    if pane_title.startswith(_CCSLACK_TITLE_PREFIX):
        stamped = pane_title[len(_CCSLACK_TITLE_PREFIX) :].strip()
        _ensure_registered()
...
def resolve_launch_command(provider_name: str) -> str:
    ...
    new_env = f"CCSLACK_{provider.upper()}_COMMAND"
    ...
    # CCSLACK_GEMINI_COMMAND overrides stay fully user-controlled.
    # For ccslack-managed Gemini launches, force stable shell mode defaults.
    if provider == "gemini" and not override:
        ...
        from ccslack.providers.gemini import build_hardened_gemini_launch_command
...
def resolve_capabilities(provider_name: str | None = None) -> ProviderCapabilities:
    ...
    Reads ``CCSLACK_PROVIDER`` (or legacy ``CCBOT_PROVIDER``) from env when
    *provider_name* is not given.  Falls back to ``"claude"`` for unknown
    ...
    name = (
        ...
        if provider_name is not None
            ...
            os.environ.get("CCSLACK_PROVIDER")
