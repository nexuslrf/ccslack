...
from ccslack.terminal_parser import UIPattern, extract_interactive_content
from ccslack.utils import atomic_write_json, ccslack_dir
...
_GEMINI_BUILTINS: dict[str, str] = {
    ...
    # /resume excluded — collides with bot-native /resume (ccslack session picker)
...
def build_hardened_gemini_launch_command(command: str) -> str:
    """Wrap Gemini launch command with ccslack-managed stability settings.

    Gemini reads this path as "system settings", so it overrides workspace/user
    settings and reliably disables interactive-shell PTY mode for ccslack runs.
    If the settings file cannot be written, returns the original command.
    """
    settings_path = ccslack_dir() / _GEMINI_SYSTEM_SETTINGS_FILE
    try:
        atomic_write_json(
            settings_path,
            {
                "tools": {
                    "shell": {
                        # Disable node-pty interactive shell mode in ccslack-managed Gemini runs.
