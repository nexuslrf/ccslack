...
class SessionResolver:
    ...
    def find_channels_for_session(
        self,
        session_id: str,
    ) -> list[tuple[str, str]]:
        """Find all (channel_id, window_id) pairs whose window maps to this session."""
        result: list[tuple[str, str]] = []
        for channel_id, window_id in thread_router.iter_channel_bindings():
            state = window_store.window_states.get(window_id)
