...
CCSLACK_CREATED_WINDOW_ORIGIN = "ccslack_created"
...
class WindowState:
    ...
    Attributes:
        ...
        origin: Lifecycle origin. Manual/external windows are never auto-killed by ccslack.
    ...
    # Gemini window lacks ccslack's hardened shell settings (issue #86).
    # Persisted so the warning is not re-shown on every bot restart.
    gemini_external_warned: bool = False
    # Slack ts of the pinned status message in the bound channel. Persisted so
    # ``chat.update`` can target the right message after bot restarts.
