...
class NormalizedHookEvent:
    """Provider-neutral hook event safe for ccslack state files."""
...
class HookAdapter(Protocol):
    ...
    ``installable_events`` covers what ccslack actually installs in user config —
    a subset chosen for the lifecycle signals ccslack acts on. The two differ on
    ...
    install_managed_by_ccslack: bool
