...
The adapters validate common fields, map native event names to ccslack's canonical
...
class ClaudeHookAdapter:
    ...
    install_managed_by_ccslack = True
...
class PiHookAdapter:
    ...
    # Pi hooks are installed by cc-thingz hook-runner, not ccslack.
    installable_events: tuple[str, ...] = ()
    install_managed_by_ccslack = False
...
class CodexHookAdapter:
    ...
    # Only the lifecycle signals ccslack acts on — the rest are accepted by
