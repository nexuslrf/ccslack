...
class UserPreferences:
    ...
    def __init__(self, *, schedule_save: Callable[[], None]) -> None:
        self.user_dir_favorites: dict[str, dict[str, list[str]]] = {}
        self.user_window_offsets: dict[str, dict[str, int]] = {}
    ...
    def prune_stale_offsets(self, known_window_ids: set[str]) -> bool:
        ...
        for uid, offsets in self.user_window_offsets.items():
            ...
            for wid in stale:
                logger.info("Pruning stale offset: user %s, window %s", uid, wid)
                del offsets[wid]
                changed = True
            if not offsets:
