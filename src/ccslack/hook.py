...
Config directory resolution uses utils.ccslack_dir() (shared with config.py).
...
from ccslack.hooks.adapters import (
    detect_provider_from_payload,
    get_hook_adapter,
)
from ccslack.hooks.model import ProviderName
...
_CURRENT_HOOK_MARKER = "ccslack.main hook"
# Older installs used the console script name directly.
_PATH_HOOK_MARKER = "ccslack hook"
...
# Hook event types ccslack handles (order matters for status display)
...
def _current_hook_command(provider_name: str = "claude") -> str:
    ...
    command = f"{shlex.quote(sys.executable)} -m ccslack.main hook"
...
def _is_any_ccslack_hook_command(command: str) -> bool:
...
def _has_ccslack_hook(settings: dict, event_type: str) -> bool:
    """Check if ccslack hook (or legacy ccbot hook) is installed."""
    return _has_matching_hook(settings, event_type, _is_any_ccslack_hook_command)


def get_installed_events(settings: dict) -> dict[str, bool]:
    """Return installation status for each expected hook event type."""
    return {event: _has_ccslack_hook(settings, event) for event in _HOOK_EVENT_TYPES}
...
def _json_hook_command_predicate(provider_name: str) -> Callable[[str], bool]:
    """Build predicate for provider-specific ccslack hook commands.
    ...
    def _predicate(command: str) -> bool:
        return _is_any_ccslack_hook_command(command) and needle in f" {command} "
...
def _install_json_hooks(
    ...
    """Install ccslack command hooks into a JSON settings file.
    ...
    try:
        ...
        # paying that cost on `ccslack --help`, only at hook-install.
...
# Current Codex flag is ``hooks``; ``codex_hooks`` is the deprecated alias
# (Codex prints a warning if the old name is still in config.toml). We migrate
# any existing legacy entry to the new name when writing.
_CODEX_HOOKS_KEY_RE = re.compile(r"^\s*hooks\s*=\s*(\S+)", re.MULTILINE)
_CODEX_LEGACY_HOOKS_KEY_RE = re.compile(
    r"^[ \t]*codex_hooks[ \t]*=[ \t]*\S+[ \t]*\n?", re.MULTILINE
)
...
def _ensure_codex_feature_flag() -> int:
    ...
    Detects an existing ``hooks =`` line (spacing-tolerant). If it's already
    truthy, no-op. If it's explicitly false, warn and refuse to overwrite —
    the user opted out. Otherwise insert under ``[features]``. Any deprecated
    ``codex_hooks =`` entry is stripped so Codex stops printing the warning.
    """
    ...
    # Strip the deprecated alias if present — Codex warns about it on every
    # invocation. Migration is silent: we keep going to insert / verify the
    # new flag below.
    migrated_text = _CODEX_LEGACY_HOOKS_KEY_RE.sub("", text)
    legacy_removed = migrated_text != text
    text = migrated_text
...
def _uninstall_json_hooks(path: Path, provider_name: str) -> int:
    """Remove provider-specific ccslack hooks from a JSON settings file."""
    settings = _load_json_settings(path)
    if settings is None:
        return 1
    if not settings:
...
def _install_hook(provider_name: str = "claude") -> int:  # noqa: PLR0912
    """Install ccslack hooks for all event types into provider settings.
    ...
    for event_type in _HOOK_EVENT_TYPES:
        ...
        has_known = _has_matching_hook(
            settings, event_type, _is_any_ccslack_hook_command
        )
...
def _uninstall_hook(provider_name: str = "claude") -> int:  # noqa: PLR0911
    """Remove ccslack hooks from provider settings.
    ...
    # Check if any ccslack hooks are installed
    any_installed = any(
        _has_ccslack_hook(settings, event) for event in _HOOK_EVENT_TYPES
    )
    if not any_installed:
        print("Hook not installed — nothing to uninstall.")
        return 0

    # Remove ccslack hook entries from all event types
    ...
    for event_type in _HOOK_EVENT_TYPES:
        ...
        for entry in event_hooks:
            ...
            filtered = [
                ...
                for h in inner_hooks
                if not isinstance(h, dict)
                or not _is_any_ccslack_hook_command(h.get("command", ""))
            ]
            if filtered:
...
def _hook_status(provider_name: str = "claude") -> int:  # noqa: PLR0911
    ...
    match provider_name:
        ...
        case "pi":
            ...
            print(
                "Expected built-in hook-runner ccslack events: "
...
def _write_event(
    ...
    # Lazy: hook.py runs as `python -m ccslack.hook` from Claude Code on
    # every notification; deferring utils import until an event actually
    # fires keeps the latency-sensitive fast path lean.
    # Lazy: utils.ccslack_dir resolves $CCSLACK_DIR at runtime
    from .utils import ccslack_dir

    events_file = ccslack_dir() / "events.jsonl"
...
def _update_session_map(
    ...
    from .utils import ccslack_dir, atomic_write_json

    map_file = ccslack_dir() / "session_map.json"
...
def _process_hook_stdin(provider_name: str | None = None) -> None:
    ...
    # Codex *requires* every hook invocation to produce a JSON object on stdout
    # or it raises "invalid stop hook JSON output". ccslack is a passive
    # observer, so the empty ack ``{}`` is always correct. Emit it upfront so
    # every early-return path (parse error, unknown payload, unresolvable
    # window, etc.) still satisfies Codex's contract. Claude / Gemini / Pi
    # don't read stdout so the write is harmless for them when --provider is
    # set explicitly.
    ...
    if event == "SessionStart":
        ...
        # Codex ack already emitted upfront; no-op here.
        if detected_provider == "codex" and provider_name != "codex":
            # Provider was payload-detected as Codex (not from --provider flag)
            # — the upfront ack didn't fire. Emit it now.
            sys.stdout.write("{}\n")
            sys.stdout.flush()
        return
...
def hook_main(
    ...
    # structlog defaults to ``PrintLoggerFactory(file=sys.stdout)``, which would
