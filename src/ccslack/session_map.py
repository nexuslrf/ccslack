...
def _primary_session_grace_sec() -> float:
    raw = os.getenv("CCSLACK_NESTED_SESSION_GRACE_SEC")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "CCSLACK_NESTED_SESSION_GRACE_SEC must be a number, got %r; using default %.1f",
            raw,
...
def parse_session_map(raw: dict[str, Any], prefix: str) -> dict[str, dict[str, str]]:
    ...
    Also matches legacy "ccbot:" prefix keys when the current prefix is "ccslack:".
    ...
    legacy_prefix = _LEGACY_SESSION_PREFIX if prefix.startswith("ccslack:") else ""
...
class SessionMapSync:
    ...
    async def load_session_map(self) -> None:
        ...
        Keys in session_map are formatted as "tmux_session:window_id" (e.g. "ccslack:@12").
