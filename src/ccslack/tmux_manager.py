...
class TmuxManager:
    ...
    async def stamp_pane_title(self, window_id: str, provider_name: str) -> None:
        """Set pane title to ``ccslack:<provider>`` for instant re-detection.
    ...
    async def create_window(
        ...
        # Strip any trailing -N/-N-N suffixes first so repeated restores
        # don't accumulate "imaginaire4-2-2-2..." names.
        raw_name = window_name if window_name else path.name
        base_name = re.sub(r"(-\d+)+$", "", raw_name) or raw_name
        final_window_name = base_name
        counter = 2
        ...
        def _create_and_start() -> tuple[bool, str, str, str]:
            ...
            try:
                ...
                # Set CCSLACK_WINDOW_ID so agents can self-identify
