...
qualified window IDs (e.g. ``ccslack:@0``) matching session_map convention.
...
def sanitize_dir_name(qualified_id: str) -> str:
    ...
    Replaces colons with ``=`` so that IDs like ``ccslack:@0`` become
    ``ccslack=@0`` (filesystem-safe on all platforms).
    """
...
def _unsanitize_dir_name(dir_name: str) -> str:
    """Reverse ``sanitize_dir_name``: ``ccslack=@0`` → ``ccslack:@0``.
...
class Mailbox:
    ...
    Directory layout::
        ...
        base_dir/
          ccslack=@0/           # sanitized qualified ID
