...
| `CCSLACK_FILE_REFS` | `true` | When an agent's final answer names real project files (backtick paths, output-artifact paths, markdown links), post a **Show files** button that lists exactly those files for one-tap upload. Only files that exist under the session cwd and pass the `/ccslack send` security pipeline (no hidden/secret/gitleaks files) are surfaced. Set `false` to suppress the button. |
...
| `CCSLACK_WORKERS` | empty | router | Comma-separated `host=ssh_target[:port]` entries, e.g. `gpu1=user@gpu1,gpu2=gpu2-alias`. Optional trailing `:port` overrides that worker's link port (co-locate two workers on one host). Empty = single-host router (standalone behaviour). |
...
| `CCSLACK_TOOLCALLS` | `calls` | Global default tool-chain detail: `full` (tool call + exec result), `calls` (call only — exec result skipped, the usual noise), or `hidden` (neither). Per-channel `/ccslack toolcalls` overrides it. Legacy `CCSLACK_HIDE_TOOL_CALLS=true` still maps to `hidden` when this is unset. |
...
| `tool_call_visibility` | `default` / `full` / `calls` / `hidden` | `/ccslack toolcalls …` |
